              ...
              {products.map((p) => (
                  ...
                  <Link to={`/product/${p.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                    ...
                    <div style={{ padding: '24px' }}>
                      ...
                      <p style={{ color: '#6b7280', fontSize: '14px', marginBottom: '8px', lineHeight: '1.5', minHeight: '42px' }}>
                        {p.description}
                      </p>
                      <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '16px' }}>
                        호환: {Array.isArray(p.compatibility) ? p.compatibility.join(', ') : p.compatibility}
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                        <div style={{ fontWeight: 700, fontSize: '20px', color: '#1f2937' }}>
                          {p.discountApplied === 'Y' && typeof (p as any).discountRate === 'number' && (p as any).discountRate > 0 && (p.discountedPrice || 0) < p.price && (
                            <span style={{ fontSize: '12px', color: '#ef4444', fontWeight: 600, marginRight: '8px' }}>
                              {(p as any).discountRate}% 할인
                            </span>
                          )}
                          {(p.discountedPrice || p.price).toLocaleString()}원
                          {p.discountApplied === 'Y' && p.discountedPrice && p.discountedPrice < p.price && (
                            <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px' }}>
                              <span style={{ textDecoration: 'line-through' }}>
                                원가: {p.price.toLocaleString()}원
                              </span>
                            </div>
                          )}
                        </div>
