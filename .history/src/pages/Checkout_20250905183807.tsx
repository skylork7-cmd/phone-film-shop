        ...
        {items.map(({ product, quantity }) => (
          <div key={product.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <div>{((product.discountedPrice || product.price) * quantity).toLocaleString()}원</div>
              {product.discountApplied === 'Y' && product.discountRate && product.discountRate > 0 && product.discountedPrice && product.discountedPrice < product.price && (
              ...
              {product.discountApplied === 'Y' && product.discountedPrice && product.discountedPrice < product.price && (
                <div style={{ fontSize: 12, color: '#9ca3af', textDecoration: 'line-through' }}>
                  원가: {(product.price * quantity).toLocaleString()}원
                </div>
              )}
            </div>
          </div>
