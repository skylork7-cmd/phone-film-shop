...
export const ProductsTab: React.FC<ProductsTabProps> = ({ products, setProducts, loadProducts }) => {
  ...
  const handleDiscountRateChange = (rate: string) => {
    ...
    // 할인률만 변경하고 할인 적용 상태는 변경하지 않음
    setNewProduct({
      ...newProduct,
      discountRate: rate,
      discountedPrice: newProduct.discountApplied === 'Y' && discountRate > 0 
        ? calculateDiscountedPrice(price, discountRate).toString() 
        : price.toString()
    });
  };
  ...
  const handleDiscountAppliedChange = (applied: string) => {
    if (applied === 'N') {
      ...
      // 할인 적용을 'Y'로 변경할 때, 기존 할인률이 있으면 자동으로 계산
      const price = parseFloat(newProduct.price) || 0;
      const discountRate = parseFloat(newProduct.discountRate) || 0;
      const calculatedPrice = discountRate > 0 ? calculateDiscountedPrice(price, discountRate) : price;
  ...
  const addProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 할인 관련 유효성 검사
      if (isApplied) {
        // 할인 적용 시 할인률이 0보다 커야 함
        if (rateNum <= 0) {
          alert('할인 적용 시 할인률은 0보다 커야 합니다.');
          return;
        }

        // 할인 종료일이 현재 시간보다 커야 함
        if (endDate) {
          const endDateTime = new Date(endDate);
          const now = new Date();
          if (endDateTime <= now) {
            alert('할인 종료일은 현재 시간보다 커야 합니다.');
            return;
          }
        }
